...
namespace Models
{
	...
	public partial class Model0_EP : IGeneratedAlgorithm
	{
		...
		public Vector[] X
		{
			...
			set {
				if ((value!=null)&&(value.Length!=99)) {
					throw new ArgumentException(((("Provided array of length "+value.Length)+" when length ")+99)+" was expected for variable \'X\'");
				}
				this.X_field = value;
		...
		public double[] y
		{
			...
			set {
				if ((value!=null)&&(value.Length!=99)) {
					throw new ArgumentException(((("Provided array of length "+value.Length)+" when length ")+99)+" was expected for variable \'y\'");
				}
				this.y_field = value;
				this.numberOfIterationsDone = 0;
		...
		private void Changed_numberOfIterations_X_y(int numberOfIterations)
		{
			...
			DenseVector vVector4 = DenseVector.FromArray(new double[2] { 0.0, 0.0 });
			PositiveDefiniteMatrix vPositiveDefiniteMatrix1 = new PositiveDefiniteMatrix(new double[2, 2] { { 1.0, 0.0 }, { 0.0, 1.0 } });
			VectorGaussian phi_F = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(vVector4.Count));
			this.phi_marginal_F = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(vVector4.Count));
			...
			phi_F = VectorGaussianFromMeanAndVarianceOp.SampleAverageConditional(vVector4, vPositiveDefiniteMatrix1);
			VectorGaussian[] phi_rep_F;
			// Create array for 'phi_rep' Forwards messages.
			phi_rep_F = new VectorGaussian[99];
			for(int dataRange = 0; dataRange<99; dataRange++) {
				phi_rep_F[dataRange] = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(vVector4.Count));
			}
			// Create array for replicates of 'phi_rep_F_dataRange__BMean'
			DenseVector[] phi_rep_F_dataRange__BMean = new DenseVector[99];
			for(int dataRange = 0; dataRange<99; dataRange++) {
			...
			PositiveDefiniteMatrix[] phi_rep_F_dataRange__BVariance = new PositiveDefiniteMatrix[99];
			for(int dataRange = 0; dataRange<99; dataRange++) {
			...
			precision_rep_F = new Gamma[99];
			for(int dataRange = 0; dataRange<99; dataRange++) {
				precision_rep_F[dataRange] = Gamma.Uniform();
			}
			// Create array for replicates of 'mean_B'
			Gaussian[] mean_B = new Gaussian[99];
			for(int dataRange = 0; dataRange<99; dataRange++) {
				mean_B[dataRange] = Gaussian.Uniform();
			}
			VectorGaussian[] phi_rep_B;
			// Create array for 'phi_rep' Backwards messages.
			phi_rep_B = new VectorGaussian[99];
			for(int dataRange = 0; dataRange<99; dataRange++) {
				phi_rep_B[dataRange] = ArrayHelper.MakeUniform<VectorGaussian>(VectorGaussian.Uniform(vVector4.Count));
			}
		...
		private void Changed_y()
		{
			...
			this.y_marginal_F = new DistributionStructArray<Gaussian,double>(99);
			for(int dataRange = 0; dataRange<99; dataRange++) {
